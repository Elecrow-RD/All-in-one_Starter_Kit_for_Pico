...
// 定义红色LED连接的引脚为18
#define MPU6050_WHO_AM_I 0x75    // Read Only，MPU-6050的“WHO AM I”寄存器地址，用于设备识别，该寄存器为只读
#define MPU6050_PWR_MGMT_1 0x6B  // Read and Write，MPU-6050的电源管理寄存器1地址，可读可写
#define MPU_ADDRESS 0x68        // MPU-6050设备的I2C地址
...
// L17 课程的初始化函数，用于对 MPU - 6050 传感器进行初始化配置
void L17_init() {
    // 设置 I2C 总线 Wire1 的数据引脚（SDA）为 2，用于数据传输
    Wire1.setSDA(2);
    // 设置 I2C 总线 Wire1 的时钟引脚（SCL）为 3，用于同步数据传输
    Wire1.setSCL(3);
    // 启动 Wire1 这个 I2C 总线的通信功能，为后续与 MPU - 6050 通信做准备
    Wire1.begin();

    // 开始与 MPU - 6050 进行通信，指定 MPU - 6050 的设备地址为 MPU_ADDRESS
    Wire1.beginTransmission(MPU_ADDRESS);
    // 向 MPU - 6050 写入“WHO AM I”寄存器的地址，用于确认设备的身份
    Wire1.write(MPU6050_WHO_AM_I);
    // 写入数据 0x00，可能是查询设备 ID 操作的一部分
    Wire1.write(0x00);
    // 结束这次与 MPU - 6050 的数据传输
    Wire1.endTransmission();

    // 再次开始与 MPU - 6050 进行通信，指定设备地址
    Wire1.beginTransmission(MPU_ADDRESS);
    // 向 MPU - 6050 写入电源管理寄存器 1 的地址，用于设置电源管理模式
    Wire1.write(MPU6050_PWR_MGMT_1);
    // 写入数据 0x00，将 MPU - 6050 从睡眠模式唤醒或设置默认电源管理状态
    Wire1.write(0x00);
    // 结束这次与 MPU - 6050 的数据传输
    Wire1.endTransmission();

    // 将引脚 14 设置为输入模式，用于检测是否有退出课程的信号
    pinMode(14, INPUT);
}

// L17 课程的主逻辑函数，实现从 MPU - 6050 传感器读取数据并显示的功能
void L17_Curriculum() {
    // 调用 L17 课程的初始化函数，完成 MPU - 6050 传感器和引脚的初始化
    L17_init();
    // 进入一个无限循环，持续读取 MPU - 6050 传感器的数据
    while (1) {
        ...
        if (digitalRead(14)) {
            ...
            Curriculum_flag = 0;
            // 跳出当前的无限循环，结束 L17 课程的逻辑执行
            break;
        }

        // 开始与 MPU - 6050 进行通信，指定设备地址为 0x68
        Wire1.beginTransmission(0x68);
        // 向 MPU - 6050 写入寄存器地址 0x3B，该地址对应加速度等数据的起始地址
        Wire1.write(0x3B);
        // 结束这次传输，但不释放 I2C 总线，以便后续请求数据
        Wire1.endTransmission(false);
        // 从 MPU - 6050 请求 14 个字节的数据，包含加速度、温度和角速度信息
        Wire1.requestFrom(0x68, 14, true);
        // 等待直到接收到 14 个字节的数据
        while (Wire1.available() < 14);

        // 定义变量用于存储从 MPU - 6050 读取的原始数据
        int16_t axRaw, ayRaw, azRaw, gxRaw, gyRaw, gzRaw, Temperature;

        // 从 I2C 总线读取加速度 X 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        axRaw = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取加速度 Y 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        ayRaw = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取加速度 Z 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        azRaw = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取温度数据的高 8 位和低 8 位数据，并组合成 16 位数据
        Temperature = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取角速度 X 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        gxRaw = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取角速度 Y 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        gyRaw = Wire1.read() << 8 | Wire1.read();
        // 从 I2C 总线读取角速度 Z 轴的高 8 位和低 8 位数据，并组合成 16 位数据
        gzRaw = Wire1.read() << 8 | Wire1.read();

        // 将加速度 X 轴的原始数据转换为实际加速度值，单位为 G（重力加速度）
                // 将加速度 X 轴的原始数据转换为实际加速度值，单位为 G（重力加速度）
        float acc_x = axRaw / 16384.0;
        // 将加速度 Y 轴的原始数据转换为实际加速度值，单位为 G（重力加速度）
        float acc_y = ayRaw / 16384.0;
        // 将加速度 Z 轴的原始数据转换为实际加速度值，单位为 G（重力加速度）
        float acc_z = azRaw / 16384.0;

        // 使用黑色填充整个屏幕，清除之前显示的内容
        gfx.fillScreen(TFT_BLACK);
        // 设置文本输出的起始光标位置为 (0, 0)，并设置字体大小为 2
        gfx.setCursor(0, 0, 2);
        // 在屏幕上打印 X 轴的加速度值，保留两位小数
        gfx.printf("X: %.2f\n", acc_x);
        // 在屏幕上打印 Y 轴的加速度值，保留两位小数
        gfx.printf("Y: %.2f\n", acc_y);
        // 在屏幕上打印 Z 轴的加速度值，保留两位小数
        gfx.printf("Z: %.2f\n", acc_z);

        // 延时 100 毫秒，控制数据读取和显示的刷新频率
        delay(100);
    }
}
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L17 课程模式
    if (Curriculum_flag == 1) {
        // 调用 L17_Curriculum 函数，执行 L17 课程的具体逻辑，如从 MPU - 6050 传感器读取数据并显示
        L17_Curriculum();
    }
