...
// 定义一个宏，将SOUND_PIN 定义为29，用于代表连接声音传感器的引脚
#define SOUND_PIN 29
// 定义一个整型变量buzzerPin，用于连接蜂鸣器的引脚，赋值为10
int buzzerPin = 10;
...
// L16 课程的初始化函数，在课程开始时执行一次，用于初始化硬件引脚和屏幕显示
void L16_init() {
    // 将 SOUND_PIN 引脚设置为输入模式，用于读取声音传感器的模拟信号
    pinMode(SOUND_PIN, INPUT);
    // 将 buzzerPin 引脚设置为输出模式，用于控制蜂鸣器发声
    pinMode(buzzerPin, OUTPUT);
    // 将引脚 14 设置为输入模式，可能用于检测退出课程的信号
    pinMode(14, INPUT);
    // 使用黑色填充屏幕，清空屏幕上之前的显示内容
    gfx.fillScreen(TFT_BLACK);
}

// L16 课程的主逻辑函数，实现声音检测和蜂鸣器控制功能
void L16_Curriculum() {
    // 调用 L16 课程的初始化函数，完成硬件引脚和屏幕的初始化
    L16_init();
    // 进入一个无限循环，持续进行声音检测和处理
    while (1) {
        ...
        if (digitalRead(14)) {
            ...
            Curriculum_flag = 0;
            // 跳出当前的无限循环，结束 L16 课程的逻辑执行
            break;
        }

        // 从 SOUND_PIN 引脚读取模拟值，并通过串口打印该值，方便调试
        Serial.println(analogRead(SOUND_PIN));
        // 判断读取到的模拟值是否大于等于 300，如果是则认为检测到声音
        if (analogRead(SOUND_PIN) >= 300) {
            // 在串口输出提示信息，表示检测到声音
            Serial.println("[-] Detect Sound!");
            // 让蜂鸣器以 1300Hz 的频率发声
            tone(buzzerPin, 1300);
            // 延时 1000 毫秒，即让蜂鸣器持续发声 1 秒
            delay(1000);
            // 再次从 SOUND_PIN 引脚读取模拟值，并通过串口打印该值
            Serial.println(analogRead(SOUND_PIN));
            // 进入一个内层无限循环，持续检测声音是否持续存在
            while (1) {
                ...
                // 判断当前读取到的模拟值是否仍大于等于 300，如果是则表示声音还在持续
                if (analogRead(SOUND_PIN) >= 300) {
                    // 通过串口打印当前读取到的模拟值
                    Serial.println(analogRead(SOUND_PIN));
                } 
                // 如果模拟值小于 300，则表示声音已经停止
                else {
                    // 停止蜂鸣器发声
                    noTone(buzzerPin);
                    // 跳出内层的无限循环，继续外层循环进行声音检测
                    break;
                }
            }
        }
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L16 课程模式
    if (Curriculum_flag == 1) {
        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);
        // 调用 L16_Curriculum 函数，执行 L16 课程的具体逻辑，如声音检测和蜂鸣器控制
        L16_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    }
