...
// L21 课程的初始化函数，用于对相关硬件引脚进行初始化设置和屏幕清理操作
void L21_init() {
    // 将蜂鸣器连接的引脚设置为输出模式，以便后续控制蜂鸣器发声
...
// L21 课程的主逻辑函数，实现烟雾检测并根据检测结果控制蜂鸣器发声的功能
void L21_Curriculum() {
    // 调用 L21 课程的初始化函数，完成硬件引脚和屏幕的初始化
    L21_init();
    // 进入一个无限循环，持续进行烟雾检测和退出信号检测
    while (1) {
        ...
        // 定义两个浮点型变量，分别用于存储传感器的电压值和模拟读数
        float sensor_volt;
        float sensorValue;
        // 从气体传感器连接的引脚读取模拟值
        sensorValue = analogRead(gas_pin);
        // 将模拟读数转换为对应的电压值，假设参考电压为 5V
        sensor_volt = sensorValue / 1024 * 5.0;
        // 通过串口打印传感器的电压值及单位
        Serial.print("sensor_volt = ");
        Serial.print(sensor_volt);
        Serial.println("V");
        // 判断传感器的电压值是否大于 1.0V，假设该阈值表示检测到烟雾
        if (sensor_volt > 1.0) {
            // 如果检测到烟雾，使蜂鸣器以 1300Hz 的频率发声
            tone(buzzerPin, 1300);
        } else {
            // 如果未检测到烟雾，停止蜂鸣器发声
            noTone(buzzerPin);
        }
        // 延时 100 毫秒，控制检测和判断的频率
        delay(100);
    }
}
...
// loop 函数是程序的主循环，会不断重复执行，用于处理各种周期性任务和状态判断
void loop() {
    // 调用 LittlevGL 图形库的定时器处理函数，用于处理界面的定时任务，如动画更新、界面刷新等
    lv_timer_handler();

    // 调用 Scan_button 函数，用于扫描按键状态，以检测是否有按键被按下或释放，从而处理用户的输入操作
    Scan_button();

    // 延时 10 毫秒，控制循环的执行频率，避免程序运行过快导致资源过度占用
    delay(10);

    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L21 课程模式
    if (Curriculum_flag == 1) {
        // 将引脚 0 置为低电平，可能用于关闭某个外部设备或切换到特定的工作状态
        digitalWrite(0, LOW);
        // 调用 L21_Curriculum 函数，执行 L21 课程的具体逻辑，即进行烟雾检测并根据结果控制蜂鸣器发声
        L21_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    } else {
        // 如果不处于课程模式，停止引脚 10 上的音调输出，避免蜂鸣器持续发声
        noTone(10);
    }

    // 判断游戏 1 标志位 game_1 是否为 1，如果为 1 则表示进入游戏 1 模式
    if (game_1 == 1) {
        // 使用黑色填充整个屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 将引脚 0 置为低电平，可能用于关闭某个外部设备或切换到特定的工作状态
    ...
    // 判断游戏 3 标志位 game_3 是否为 1，如果为 1 则表示进入游戏 3 模式
    if (game_3 == 1) {
        // 使用黑色填充整个屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 将引脚 0 置为低电平，可能用于关闭某个外部设备或切换到特定的工作状态
