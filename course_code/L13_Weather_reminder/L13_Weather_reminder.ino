...
// 创建一个 DHT20 传感器对象 DHT，使用 Wire1 作为 I2C 通信总线
// DHT20 是一款温湿度传感器，可通过 I2C 接口与微控制器进行通信
// 这里将 Wire1 作为通信通道，方便与其他使用 Wire 的设备区分开
DHT20 DHT(&Wire1);

// 使用预处理指令 #define 定义一个宏 Red_LED，将其值设定为 18
// 该宏代表红色 LED 连接到微控制器的引脚编号
// 在后续代码中可以利用这个宏来控制红色 LED 的亮灭等状态
#define Red_LED 18

// 定义一个宏 Yellow_LED，将其值设定为 20
// 该宏代表黄色 LED 连接到微控制器的引脚编号
// 方便在代码里通过此宏对黄色 LED 进行操作
#define Yellow_LED 20

// 定义一个宏 Green_LED，将其值设定为 19
// 该宏代表绿色 LED 连接到微控制器的引脚编号
// 可用于控制绿色 LED 的状态
#define Green_LED 19

// 定义一个宏 Buzzer_PIN，将其值设定为 10
// 该宏代表蜂鸣器连接到微控制器的引脚编号
// 后续代码可以使用此宏对蜂鸣器进行控制，例如发出声音等操作
...
void L13_init() {
  ...
  // 初始化 LED 引脚
  pinMode(Red_LED, OUTPUT);     // 设置红色 LED 引脚为输出模式
  pinMode(Yellow_LED, OUTPUT);  // 设置黄色 LED 引脚为输出模式
  pinMode(Green_LED, OUTPUT);   // 设置绿色 LED 引脚为输出模式

  // 初始化蜂鸣器引脚
  pinMode(Buzzer_PIN, OUTPUT);    // 设置蜂鸣器引脚为输出模式
  digitalWrite(Buzzer_PIN, LOW);  // 初始化为低电平，关闭蜂鸣器

  // 初始化 I2C 总线用于 DHT20
  Wire1.setSDA(2);  // 设置 DHT20 的 I2C 数据引脚
  Wire1.setSCL(3);  // 设置 DHT20 的 I2C 时钟引脚
  Wire1.begin();    // 启动用于 DHT20 的 I2C 总线
  DHT.begin();      // 初始化 DHT20 传感器
  pinMode(14, INPUT);
}
void L13_Curriculum() {
  L13_init();
  while (1) {
    ...
    // 每 1000 毫秒（1 秒）读取一次 DHT20 数据
    if (millis() - DHT.lastRead() >= 1000) {
      int status = DHT.read();                     // 读取 DHT20 传感器数据
      if (status == DHT20_OK) {                    // 如果读取成功
        float temperature = DHT.getTemperature();  // 获取温度值
        float humidity = DHT.getHumidity();        // 获取湿度值
        ...
        if (temperature > 30) {
          ...
          digitalWrite(Red_LED, HIGH);   // 点亮红色 LED
          gfx.setCursor(0, 0);           // 设置文本光标位置
          gfx.print("Hot temperature");  // 打印高温提示
        } else if (temperature > 25 && temperature <= 30) {
          digitalWrite(Red_LED, LOW);
          digitalWrite(Yellow_LED, HIGH);  // 点亮黄色 LED
          gfx.setCursor(0, 0);             // 设置文本光标位置
          gfx.print("High temperature");   // 打印较高温提示
        } else {
          digitalWrite(Red_LED, LOW);     // 关闭红色 LED
          digitalWrite(Yellow_LED, LOW);  // 关闭黄色 LED
          gfx.setCursor(0, 0);            // 设置文本光标位置
          gfx.print("temperature: ");     // 打印温度值标签
          gfx.print(temperature, 1);      // 打印温度，保留 1 位小数
          gfx.print(" C");                // 打印温度单位
        }

        gfx.setCursor(0, 20);  // 设置文本光标位置到下一行
        // 根据湿度值控制蜂鸣器并显示相应信息
        if (humidity < 40) {
          tone(10, 1300);        // 让蜂鸣器发声，频率 1300Hz
          gfx.print("Dry air");  // 打印空气干燥提示
        } else {
          noTone(10);               // 关闭蜂鸣器
          gfx.print("humidity: ");  // 打印湿度值标签
          gfx.print(humidity, 1);   // 打印湿度，保留 1 位小数
          gfx.print("%");           // 打印湿度单位
        }
      }
    }
    delay(100);  // 短暂延迟，防止循环占用过多 CPU 资源
  }
}
...
void setup(void) {
    ...
    // 调用 L13 课程的初始化函数，对显示、LED、蜂鸣器和 DHT20 传感器等硬件进行初始化
    L13_init();
}
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L13 课程模式
    if (Curriculum_flag == 1) {
        // 调用 L13_Curriculum 函数，执行 L13 课程的具体逻辑，如读取温湿度数据、控制 LED 和蜂鸣器等
        L13_Curriculum();
