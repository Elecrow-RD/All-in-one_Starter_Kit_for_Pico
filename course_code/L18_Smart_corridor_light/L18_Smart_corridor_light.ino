...
// 创建一个 BH1750 光照传感器对象 lightMeter，指定其 I2C 地址为 0x5c
// BH1750 是一款数字式光照传感器，可用于测量环境光照强度
BH1750 lightMeter(0x5c);

// 定义 I2C 通信的数据引脚（SDA）为 2
// SDA 引脚用于在 I2C 总线上传输数据
#define I2C_SDA 2

// 定义 I2C 通信的时钟引脚（SCL）为 3
// SCL 引脚用于在 I2C 总线上同步数据传输
#define I2C_SCL 3

// 定义声音传感器连接的引脚为 29
// SOUND_PIN 用于连接声音传感器，可通过该引脚读取声音信号
#define SOUND_PIN 29

// 定义 LED 连接的引脚为 18
// LedPin 用于连接一个 LED 灯，可通过该引脚控制 LED 的亮灭
int LedPin = 18;
...
// L18 课程的初始化函数，在课程开始时执行一次，用于初始化相关硬件和传感器
void L18_init() {
    // 设置 I2C 总线 Wire1 的数据引脚（SDA）为之前定义的 I2C_SDA 引脚
    Wire1.setSDA(I2C_SDA);
    // 设置 I2C 总线 Wire1 的时钟引脚（SCL）为之前定义的 I2C_SCL 引脚
    Wire1.setSCL(I2C_SCL);
    // 启动 I2C 总线 Wire1 的通信功能，为后续与 BH1750 光照传感器通信做准备
    Wire1.begin();

    // 尝试初始化 BH1750 光照传感器，设置为连续高分辨率模式，指定设备地址为 0x5c，并使用 Wire1 进行通信
    if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE, 0x5c, &Wire1)) {
        // 若初始化成功，通过串口打印初始化成功信息
        Serial.println(F("BH1750 Advanced begin"));
    } else {
        // 若初始化失败，通过串口打印初始化错误信息
        Serial.println(F("Error initialising BH1750"));
    }

    // 将 LED 连接的引脚设置为输出模式，以便控制 LED 的亮灭
    pinMode(LedPin, OUTPUT);
    // 将声音传感器连接的引脚设置为输入模式，用于读取声音信号
    pinMode(SOUND_PIN, INPUT);
    // 将引脚 14 设置为输入模式，用于检测是否有退出课程的信号
    pinMode(14, INPUT);
}

// L18 课程的主逻辑函数，实现根据光照强度和声音信号控制 LED 亮灭的功能
void L18_Curriculum() {
    // 调用 L18 课程的初始化函数，完成硬件和传感器的初始化
    L18_init();
    // 使用黑色填充屏幕，清空屏幕上之前的显示内容
    gfx.fillScreen(TFT_BLACK);

    // 进入一个无限循环，持续进行光照强度检测和 LED 控制
    while (1) {
        ...
        if (digitalRead(14)) {
            ...
            Curriculum_flag = 0;
            // 跳出当前的无限循环，结束 L18 课程的逻辑执行
            break;
        }

        // 检查 BH1750 光照传感器的测量数据是否准备好
        if (lightMeter.measurementReady(true)) {
            // 从 BH1750 光照传感器读取光照强度值
            ...
            // 判断光照强度是否大于等于 100 勒克斯
            if (lux >= 100) {
                // 如果光照强度足够强，将 LED 引脚置为低电平，即关闭 LED
                digitalWrite(LedPin, LOW);
            } else {
                // 如果光照强度较弱，进入一个内层循环，持续检测声音信号
                while (1) {
                    // 再次检测引脚 14 的输入信号，如果为高电平
                    if (digitalRead(14)) {
                        // 调用 LittlevGL 的定时器处理函数，处理界面定时任务
                        lv_timer_handler();
                        // 使当前屏幕无效，强制刷新界面显示
                    ...
                    // 定义一个静态布尔变量，用于记录声音是否持续存在
                    static bool isSoundContinuing = false;

                    // 检测声音传感器的输入信号，如果为高电平，表示检测到声音
                    if (digitalRead(SOUND_PIN)) {
                        // 将 LED 引脚置为高电平，即打开 LED
                        digitalWrite(LedPin, HIGH);
                        // 将声音持续标志设置为 true
                        isSoundContinuing = true;
                    } else {
                        // 如果没有检测到声音
                        if (isSoundContinuing) {
                            // 如果之前检测到声音且声音持续标志为 true，延时 10000 毫秒（即 10 秒）
                            delay(10000);
                            // 将声音持续标志设置为 false
                            isSoundContinuing = false;
                        }
                        // 将 LED 引脚置为低电平，即关闭 LED
                        digitalWrite(LedPin, LOW);
                    }
                }
            }
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L18 课程模式
    if (Curriculum_flag == 1) {
        // 将引脚 0 置为低电平，可能用于控制某个外部设备的关闭
        digitalWrite(0, LOW);
        // 调用 L18_Curriculum 函数，执行 L18 课程的具体逻辑，如根据光照和声音控制 LED 灯
        L18_Curriculum();
        // 将引脚 0 置为高电平，可能用于控制某个外部设备的开启
        digitalWrite(0, HIGH);
    } else {
        // 如果不处于 L18 课程模式，将 LED 灯对应的引脚置为低电平，即关闭 LED 灯
        digitalWrite(LedPin, LOW);
    }
