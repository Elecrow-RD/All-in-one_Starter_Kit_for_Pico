...
// L4 课程功能函数，实现特定的课程显示和交互逻辑
void L4_Curriculum() {
    // 设置文本显示大小为 2
    gfx.setTextSize(2);
    // 将屏幕填充为黑色，清空之前的显示内容
    gfx.fillScreen(TFT_BLACK);
    // 设置文本输出的起始光标位置为屏幕坐标 (100, 50)
    gfx.setCursor(100, 50);
    // 在屏幕上打印 "Hello Wrold!" 文本（此处存在拼写错误，应为 "Hello World!"）
    gfx.print("Hello Wrold!");
    // 延时 1000 毫秒（即 1 秒），让用户有时间看到显示内容
    delay(1000);
    // 再次将屏幕填充为黑色，清空之前的显示内容
    gfx.fillScreen(TFT_BLACK);
    // 重新设置文本输出的起始光标位置为屏幕坐标 (100, 50)
    gfx.setCursor(100, 50);
    // 在屏幕上打印 "BYE!" 文本
    gfx.print("BYE!");
    // 将引脚 14 设置为输入模式，用于后续检测输入信号
    pinMode(14, INPUT);
    // 延时 1000 毫秒（即 1 秒），让用户有时间看到显示内容
    delay(1000);
    // 将引脚 0 置为低电平，可能用于控制某个外部设备
    digitalWrite(0, LOW);
    // 再次将屏幕填充为黑色，清空之前的显示内容
    gfx.fillScreen(TFT_BLACK);
    ...
    // 将引脚 0 置为高电平，可能用于控制某个外部设备
    digitalWrite(0, HIGH);
}

// 程序初始化函数，在程序开始时执行一次，用于初始化各种硬件和软件组件
void setup(void) {
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L4 课程逻辑
    if (Curriculum_flag == 1) {
        // 调用 L4_Curriculum 函数，执行 L4 课程的具体逻辑
        L4_Curriculum();
        // 将课程标志位设置为 0，表示课程执行完毕
        Curriculum_flag = 0;
        // 设置 ui_Label11 标签的文本为 "OFF"
        lv_label_set_text(ui_Label11, "OFF");
    }
