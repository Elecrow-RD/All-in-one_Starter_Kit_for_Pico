...
// 使用 const byte 定义一个常量变量 triggerPin，其值为 9。
// 这个变量用于表示超声波传感器的触发引脚（Trigger Pin）。
// 触发引脚用于向超声波传感器发送一个短脉冲信号，以此触发传感器发射超声波。
const byte triggerPin = 9;

// 使用 const byte 定义一个常量变量 echoPin，其值为 8。
// 这个变量用于表示超声波传感器的回声引脚（Echo Pin）。
// 回声引脚用于接收超声波遇到障碍物后反射回来的回声信号，
// 通过测量回声信号的时间来计算障碍物与传感器之间的距离。
const byte echoPin = 8;

// 创建一个 UltraSonicDistanceSensor 类的对象 distanceSensor。
// UltraSonicDistanceSensor 类可能是自定义的用于操作超声波传感器的类。
// 在创建对象时，将之前定义的触发引脚 triggerPin 和回声引脚 echoPin 作为参数传递给构造函数，
// 这样 distanceSensor 对象就知道应该使用哪两个引脚与超声波传感器进行通信，
// 后续可以通过该对象调用相应的方法来测量距离。
...
// 用于存储上一次执行距离测量和显示操作的时间，初始值为 0
unsigned long pre = 0;
// 定义距离测量和显示操作的时间间隔，单位为毫秒，这里设置为 400 毫秒
const long in = 400;

// L7 课程主逻辑函数，实现使用超声波传感器测量距离并在屏幕上显示的功能
void L7_Curriculum() {
    // 调用 L7 课程的初始化函数，进行显示和输入引脚的初始化
    L7_init();
    // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
    gfx.setCursor(0, 0);
    // 在屏幕上打印固定文本 "Distance: "，用于提示后续显示的是距离信息
    gfx.printf("Distance: ");

    // 进入一个无限循环，持续进行距离测量和显示操作
    while (1) {
        // 调用超声波传感器对象的 measureDistanceCm 方法，测量当前距离并将结果存储在 distance 变量中，单位为厘米
        float distance = distanceSensor.measureDistanceCm();
        // 定义两个整型变量 x 和 y，此处未使用
        int x, y;
        // 获取当前时间（自程序启动以来的毫秒数）
        ...
        if (currentMillis - pre >= in) {
            ...
            if (distance > 0) {
                // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 用黑色填充一个指定矩形区域（110, 0 为矩形左上角坐标，200 为宽度，20 为高度），用于局部清屏，清除之前显示的距离数值
                gfx.fillRect(110, 0, 200, 20, TFT_BLACK);
                // 设置更新后的距离数值的输出光标位置
                gfx.setCursor(100, 0);
                // 在屏幕上打印测量得到的距离数值，保留两位小数，并添加单位 "cm"
                gfx.printf(" %.2f cm", distance);
            }
            // 更新上一次执行时间为当前时间，为下一次时间间隔判断做准备
            pre = currentMillis;
        }
        ...
        if (digitalRead(14)) {
            ...
            // 跳出无限循环，结束 L7 课程的逻辑执行
            break;
        }
        // 延时 10 毫秒，避免过于频繁地检测输入信号
        delay(10);
    }
}
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L7 课程逻辑
    if (Curriculum_flag == 1) {
        // 调用 L7_Curriculum 函数，执行 L7 课程的具体逻辑
        L7_Curriculum();
        // 设置 ui_Label11 标签的文本为 "OFF"
        lv_label_set_text(ui_Label11, "OFF");
    }
