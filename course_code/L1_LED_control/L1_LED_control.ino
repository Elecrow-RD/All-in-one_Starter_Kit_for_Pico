...
// 该函数用于进行初始化操作，包括硬件初始化、显示初始化和UI初始化
void L1_init()
{
...
// 延迟的时间间隔，单位为毫秒
const long inter = 1000;

// loop函数，程序运行后会不断循环执行这里面的代码
// 该函数用于控制LED灯的闪烁，并根据引脚14的状态退出循环
void L1_Curriculum()
{
  // 将引脚0设置为低电平
  digitalWrite(0, LOW);
...
// setup函数，程序启动时运行一次
// 该函数用于进行初始化操作，包括串口初始化、硬件初始化、显示初始化和UI初始化
void setup(void)
{
  // 初始化串口通信，波特率为115200
  Serial.begin(115200);
  ...
  // 切换到指定屏幕，使用淡入动画，动画时长500毫秒
  _ui_screen_change(&ui_Screen2, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_Screen2_screen_init);
  // 处理LVGL的定时器事件
  lv_timer_handler();
} // loop函数是Arduino程序的主循环，会不断重复执行
void loop()
{
  // 处理LVGL的定时器事件，确保LVGL界面能够正常更新
  lv_timer_handler();
  // 扫描按钮状态，可能用于检测用户的按键操作
  Scan_button();
  // 延迟10毫秒，避免循环执行过快
  delay(10);

  // 如果Curriculum_flag标志为1，表示进入课程模式
  ...
  {
    // 如果不在课程模式，将红、绿、黄三个LED灯熄灭
    digitalWrite(Red_LED, LOW);
    digitalWrite(Green_LED, LOW);
    digitalWrite(Yellow_LED, LOW);
  }

  // 如果game_1标志为1，表示启动游戏1
  if (game_1 == 1)
  {
    // 将屏幕填充为黑色
    gfx.fillScreen(TFT_BLACK);
