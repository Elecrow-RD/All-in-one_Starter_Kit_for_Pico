...
// DHT20 是一款温湿度传感器，通过 I2C 接口与微控制器进行通信
DHT20 DHT(&Wire1);

// L10 课程初始化函数，对显示、I2C 总线、传感器和输入引脚进行初始化设置
void L10_init() {
    ...
    // 初始化 I2C 总线 Wire1，用于与 DHT20 传感器进行通信
    // 设置 I2C 总线的 SDA（数据）引脚为 2
    Wire1.setSDA(2);
    // 设置 I2C 总线的 SCL（时钟）引脚为 3
    Wire1.setSCL(3);
    // 启动 I2C 总线通信
    Wire1.begin();

    // 初始化 DHT20 传感器，为后续读取温湿度数据做准备
    DHT.begin();
...
// 用于存储上一次执行读取温湿度和显示操作的时间，初始值为 0
unsigned long pre = 0;
// 定义读取温湿度和显示操作的时间间隔，单位为毫秒，这里设置为 400 毫秒
const long in = 400;

// L10 课程的主逻辑函数，实现读取 DHT20 传感器的温湿度数据并在屏幕上显示
void L10_Curriculum() {
    // 调用 L10 课程的初始化函数，完成显示、I2C 总线、传感器和输入引脚的初始化
    L10_init();

    // 进入一个无限循环，持续执行温湿度读取和显示逻辑
    while (1) {
        // 获取当前时间，自程序启动以来的毫秒数
        ...
        if (currentMillis - pre >= in) {
            // 判断距离上一次读取 DHT20 传感器数据是否已经过去至少 1000 毫秒
            // 避免过于频繁地读取传感器数据，因为 DHT20 传感器的更新周期一般为 1 秒
            if (millis() - DHT.lastRead() >= 1000) {
                // 调用 DHT20 传感器对象的 read 方法读取温湿度数据，返回读取状态
                int status = DHT.read();
                // 判断读取状态是否为成功（DHT20_OK 表示读取成功）
                if (status == DHT20_OK) {
                    // 使用黑色填充屏幕，清空屏幕上之前显示的温湿度数据
                    gfx.fillScreen(TFT_BLACK);
                    // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                    gfx.setCursor(0, 0);
                    // 在屏幕上打印 "Humidity: " 文本，用于提示后续显示的是湿度信息
                    gfx.print("Humidity: ");
                    // 从 DHT20 传感器对象中获取湿度值，并保留 1 位小数后打印在屏幕上
                    gfx.print(DHT.getHumidity(), 1);
                    // 在屏幕上打印湿度单位 "%"
                    gfx.print("%");

                    // 设置文本输出的起始光标位置为屏幕 (0, 20) 处
                    gfx.setCursor(0, 20);
                    // 在屏幕上打印 "Temp: " 文本，用于提示后续显示的是温度信息
                    gfx.print("Temp: ");
                    // 从 DHT20 传感器对象中获取温度值，并保留 1 位小数后打印在屏幕上
                    gfx.print(DHT.getTemperature(), 1);
                    // 在屏幕上打印温度单位 " C"
                    gfx.print(" C");
                }
            }
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L10 课程逻辑
    if (Curriculum_flag == 1) {
        // 调用 L10_Curriculum 函数，执行 L10 课程的具体逻辑
        L10_Curriculum();
