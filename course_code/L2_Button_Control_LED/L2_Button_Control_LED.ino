...
void L2_Curriculum() {
  ...
  while (1) {
    // 从按钮引脚读取模拟值
    adcValue = analogRead(Button_pin);
    // 如果读取的模拟值在740到750之间
...
void setup(void) {
  ...
  // 切换到 ui_Screen2 界面，使用淡入动画效果，动画时长 500 毫秒
...
void loop() {
  ...
  // 如果 Curriculum_flag 为 1，表示进入课程模式
  if (Curriculum_flag == 1) {
    ...
    // 如果 Curriculum_flag 不为 1，将红色 LED 引脚置为低电平
    digitalWrite(Red_LED, LOW);
  }

  // 如果 game_1 为 1，表示进入游戏 1 模式
  if (game_1 == 1) {
    // 将屏幕填充为黑色
    gfx.fillScreen(TFT_BLACK);
    // 将引脚 0 置为低电平
  ...
  if (game_2 == 1) {
    ...
    // 处理 LittlevGL 的定时器事件
    lv_timer_handler();
    // 设置 ui_TabView2 的活动标签页为第 2 页，开启动画效果
