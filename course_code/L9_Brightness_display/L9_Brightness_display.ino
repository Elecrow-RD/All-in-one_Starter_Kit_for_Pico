...
// 使用预处理指令 #define 定义一个宏 ADC_pin，将其值设定为 28。
// ADC 即模拟 - 数字转换器（Analog - to - Digital Converter），
// 此宏代表连接到微控制器的 ADC 引脚编号。后续代码中使用 ADC_pin 时，
// 编译器会自动将其替换为 28，方便在代码里引用该引脚，
// 常用于读取模拟信号，例如传感器输出的连续电压值等。
#define ADC_pin 28

// 同样使用 #define 定义宏 Red_LED，值为 18。
// 该宏代表红色 LED 连接到微控制器的引脚编号。
// 在后续代码中可以利用这个宏来控制红色 LED 的亮灭等状态，
// 比如通过对该引脚输出高电平或低电平来点亮或熄灭红色 LED。
#define Red_LED 18

// 定义一个整型变量 adcValue，用于存储从 ADC 引脚读取到的模拟信号转换后的数字值。
// 微控制器的 ADC 模块会将接收到的模拟电压信号转换为对应的数字值，
// 这个数字值就会存储在 adcValue 变量中，方便后续程序进行处理和判断。
int adcValue;

// 定义一个整型变量 mappedValue，用于存储将 adcValue 经过映射处理后的值。
// 在实际应用中，ADC 读取到的值范围可能与我们期望的控制范围不一致，
// 这时就需要进行映射操作，例如将 ADC 读取的 0 - 1023 范围的值映射到 0 - 255 范围，
// 映射后的结果就会存储在 mappedValue 变量中，以便用于后续的控制逻辑，
// 如控制 LED 的亮度、电机的转速等。
int mappedValue;
...
// L9 课程初始化函数，对硬件引脚和显示进行初始化设置
void L9_init() {
    // 将 ADC 引脚设置为输入模式，以便从该引脚读取模拟信号
    pinMode(ADC_pin, INPUT);
    // 将红色 LED 引脚设置为输出模式，用于控制红色 LED 的亮度
    ...
    // 设置后续要显示的文本大小
    gfx.setTextSize(2);
    // 将引脚 14 设置为输入模式，用于检测外部输入信号，可能作为退出条件
    pinMode(14, INPUT);
}

// 用于存储上一次执行相关操作的时间，初始值为 0
unsigned long pre = 0;
// 定义执行相关操作的时间间隔，单位为毫秒，这里设置为 100 毫秒
const long in = 100;

// L9 课程的主逻辑函数，根据 ADC 读取的值控制红色 LED 亮度并显示亮度信息
void L9_Curriculum() {
    // 调用 L9 课程的初始化函数，完成硬件和显示的初始化
    L9_init();

    // 进入一个无限循环，持续执行 ADC 读取和 LED 亮度控制逻辑
    while (1) {
        // 获取当前时间，自程序启动以来的毫秒数
        ...
        if (currentMillis - pre >= in) {
            // 从 ADC 引脚读取模拟值，并将其存储在 adcValue 变量中
            adcValue = analogRead(ADC_pin);

            // 判断读取到的 ADC 值是否小于 51
            if (adcValue < 51) {
                // 如果小于 51，将红色 LED 亮度设置为 0，即关闭 LED
                analogWrite(Red_LED, 0);
                // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 在屏幕上打印 "brightness: " 文本
                gfx.print("brightness: ");
                // 在屏幕上打印亮度值 0
                gfx.print("0");
            } 
            // 判断读取到的 ADC 值是否大于 1000
            else if (adcValue > 1000) {
                // 如果大于 1000，将红色 LED 亮度设置为 255，即全亮
                analogWrite(Red_LED, 255);
                // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 用黑色填充一个指定矩形区域（120, 0 为矩形左上角坐标，100 为宽度，20 为高度），用于局部清屏
                gfx.fillRect(120, 0, 100, 20, TFT_BLACK);
                // 重新设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 在屏幕上打印 "brightness: 10" 文本
                gfx.print("brightness: 10");
            } 
            // 如果 ADC 值在 51 到 1000 之间
            else {
                // 将 ADC 值从 0 - 1023 范围映射到 0 - 255 范围，用于控制 LED 亮度
                mappedValue = map(adcValue, 0, 1023, 0, 255);
                // 根据映射后的亮度值设置红色 LED 的亮度
                analogWrite(Red_LED, mappedValue);
                // 将映射后的亮度值再从 0 - 255 范围映射到 0 - 10 范围，用于屏幕显示
                int displayValue = map(mappedValue, 0, 255, 0, 10);
                // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 用黑色填充一个指定矩形区域（120, 0 为矩形左上角坐标
                                // 100 为宽度，20 为高度），用于局部清屏，清除之前显示的亮度值
                gfx.fillRect(120, 0, 100, 20, TFT_BLACK);
                // 重新设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                gfx.setCursor(0, 0);
                // 在屏幕上打印 "brightness: " 文本
                gfx.print("brightness: ");
                // 在屏幕上打印映射到 0 - 10 范围后的亮度值
...
void setup(void) {
    ...
    // 注册输入设备驱动程序，使 LittlevGL 可以接收输入设备的
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L9 课程逻辑
    if (Curriculum_flag == 1) {
        // 调用 L9_Curriculum 函数，执行 L9 课程的具体逻辑
        L9_Curriculum();
        // 设置 ui_Label11 标签的文本为 "OFF"
        lv_label_set_text(ui_Label11, "OFF");
    } 
    // 如果课程标志位不为 1，将红色 LED 引脚置为低电平，关闭红色 LED
    else {
