...
// 定义红色LED连接的引脚为18
// 条件编译指令，根据微控制器的 Flash 内存结束地址（FLASHEND）来决定是否排除一些特殊协议
// 如果 Flash 内存为 8k 及以下（FLASHEND <= 0x1FFF，如 ATtiny85），进行如下操作
#if FLASHEND <= 0x1FFF
    // 定义宏 EXCLUDE_EXOTIC_PROTOCOLS，在后续代码中会根据此宏排除一些特殊的红外协议
    // 目的是为了节省有限的 Flash 内存，因为特殊协议可能占用较多空间
    #define EXCLUDE_EXOTIC_PROTOCOLS

    // 嵌套的条件编译指令，判断是否未定义 DIGISTUMPCORE
    // 如果未定义该宏，意味着使用的不是 Digispark 核心板
    #if !defined(DIGISTUMPCORE)
        // 定义宏 EXCLUDE_UNIVERSAL_PROTOCOLS，用于排除通用的红外协议
        // 这也是为了节省程序内存，最多可节省 1000 字节
        #define EXCLUDE_UNIVERSAL_PROTOCOLS
    #endif
#endif

// 引入自定义的头文件 "PinDefinitionsAndMore.h"
// 该头文件通常包含了输入输出引脚的定义、常量以及其他相关的宏，方便在代码中使用
#include "PinDefinitionsAndMore.h"
// 引入 IRremote 库，这是一个用于处理红外遥控信号的库
// 通过该库可以实现对红外遥控器信号的接收和解码等功能
#include <IRremote.hpp>

// 条件编译指令，判断是否定义了 APPLICATION_PIN
// APPLICATION_PIN 可能是用户自定义的一个应用引脚，用于特定的功能
#if defined(APPLICATION_PIN)
    // 如果定义了 APPLICATION_PIN，则将 DEBUG_BUTTON_PIN 定义为 APPLICATION_PIN
    // DEBUG_BUTTON_PIN 用于控制是否打印接收数据的时间信息，方便调试程序
    #define DEBUG_BUTTON_PIN APPLICATION_PIN
#else
    // 如果未定义 APPLICATION_PIN，则将 DEBUG_BUTTON_PIN 定义为 1
    #define DEBUG_BUTTON_PIN 1
#endif

// 创建一个舵机对象 myservo，用于控制舵机的转动
Servo myservo;
// 定义一个整型变量 pos，用于存储舵机的目标角度，初始值为 0
int pos = 0;
// 定义一个整型变量 value，可能用于存储从红外遥控器接收到的某个数值，初始值为 0
int value = 0;

// 定义一个 String 类型的变量 num
// 用于存储从遥控器输入的数字字符，将这些字符拼凑成一个完整的角度值
String num;
// 定义一个 String 类型的变量 num1
// 用于辅助显示舵机角度信息，可能在显示界面时使用
String num1;
// 贪吃蛇结构体定义
...
void IR_end_gfx_init() {
    // 结束红外接收操作，释放相关资源
    IrReceiver.end();
    // 设置 SPI 通信的 TX（发送）引脚为 7
    SPI.setTX(7);
    // 设置 SPI 通信的 SCK（时钟）引脚为 6
    SPI.setSCK(6);
    // 启动 SPI 通信，为后续图形显示做准备
    SPI.begin();
    // 初始化图形显示设备
    gfx.begin();
    // 用黑色填充屏幕，清空屏幕上之前的显示内容
    gfx.fillScreen(TFT_BLACK);
}

// 结束图形显示相关操作，开始红外初始化操作
void gfx_end_IR_init() {
    // 如果 Flash 内存为 16k 及以上（通过判断 FLASHEND 是否大于等于 0x3FFF）
    // 将 DEBUG_BUTTON_PIN 引脚配置为输入上拉模式
    #if FLASHEND >= 0x3FFF
        pinMode(DEBUG_BUTTON_PIN, INPUT_PULLUP);
    #endif
    // 初始化红外接收器，使用引脚 11 接收红外信号，并启用 LED 反馈功能
...
// 显示输入的数字
void print_num() {
    // 调用 IR_end_gfx_init 函数，结束红外操作并初始化图形显示
    IR_end_gfx_init();
    // 设置文本输出的起始光标位置为 (0, 1)
    gfx.setCursor(0, 1);
    // 打印一串空格，用于清空之前在该位置显示的内容
    gfx.print("                ");
    // 重新设置文本输出的起始光标位置为 (0, 1)
    gfx.setCursor(0, 1);
    // 在屏幕上显示从遥控器输入并存储在 num 变量中的数字
    gfx.print(num);
    // 调用 gfx_end_IR_init 函数，结束图形显示操作并初始化红外接收
    gfx_end_IR_init();
}

// L14 课程的初始化函数，对显示、舵机控制引脚和输入引脚进行初始化设置
void L14_init() {
    // 用黑色填充屏幕，清空屏幕上之前的显示内容
    gfx.fillScreen(TFT_BLACK);
    // 设置后续要显示的文本大小
    ...
    // 将引脚 0 设置为输出模式
    pinMode(0, OUTPUT);
    // 将引脚 0 置为高电平，可能用于控制某个外部设备的开启，如点亮背光等
    digitalWrite(0, HIGH);
    // 注释掉的代码，原本用于初始化舵机相关参数，当前不执行
    // myservo.attach(13, 650, 2620);
    // 在屏幕上打印 "Servo Close" 文本，提示舵机处于关闭状态
    gfx.print("Servo Close");
    // 注释掉的代码，原本用于设置舵机的初始角度，当前不执行
    // myservo.write(180);
    // 将引脚 14 设置为输入模式，用于检测外部输入信号，可能作为退出课程的条件
    pinMode(14, INPUT);
}

// L14 课程的主逻辑函数，实现通过红外遥控器控制舵机转动并在屏幕上显示相关信息
void L14_Curriculum() {
    // 调用 L14 课程的初始化函数，完成显示、舵机控制引脚和输入引脚的初始化
    L14_init();
    ...
    while (1) {
        ...
        if (digitalRead(14)) {
            ...
            digitalWrite(0, HIGH);
            // 设置 ui_Label11 标签的文本为 "OFF"，可能用于提示课程已结束
            lv_label_set_text(ui_Label11, "OFF");
            // 跳出当前的无限循环，结束 L14 课程的逻辑执行
            break;
        }
        ...
        if (IrReceiver.decode()) {
            ...
            if (IrReceiver.decodedIRData.flags & IRDATA_FLAGS_WAS_OVERFLOW) {
                ...
                // 提示用户尝试增加 RAW_BUFFER_LENGTH 值以解决溢出问题
                Serial.println(F("Try to increase the \"RAW_BUFFER_LENGTH\" value of " STR(RAW_BUFFER_LENGTH) " in " __FILE__));
            } else {
                // 根据接收到的不同红外指令执行相应操作
                switch (IrReceiver.decodedIRData.command) {
                    case 0x45:
                        // 当接收到指令 0x45 时，在串口打印对应按键信息
                    ...
                    case 0x40:
                        ...
                        // 调用 IR_end_gfx_init 函数，结束红外操作并初始化图形显示
                        IR_end_gfx_init();
                        // 将舵机对象 myservo 连接到引脚 13，并指定舵机脉冲信号的最小值为 400，最大值为 2520
                        myservo.attach(13, 400, 2520);
                        // 控制舵机旋转到 180 度位置
                        myservo.write(180);
                        // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                        gfx.setCursor(0, 0);
                        // 在屏幕上打印 "Servo Open" 文本，提示舵机已打开
                        gfx.print("Servo Open");
                        // 调用 gfx_end_IR_init 函数，结束图形显示操作并初始化红外接收
                        gfx_end_IR_init();
                        break;
                    case 0x43:
                        ...
                        // 调用 IR_end_gfx_init 函数，结束红外操作并初始化图形显示
                        IR_end_gfx_init();
                        // 分离舵机对象与引脚的连接，停止对舵机的控制
                        myservo.detach();
                                                // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                        gfx.setCursor(0, 0);
                        // 在屏幕上打印 "Servo Close" 文本，提示舵机已关闭
                        gfx.print("Servo Close");
                        // 调用 gfx_end_IR_init 函数，结束图形显示操作并初始化红外接收
                        gfx_end_IR_init();
                        break;
                    ...
                    case 0x09:
                        ...
                        // 调用 IR_end_gfx_init 函数，结束红外操作并初始化图形显示
                        IR_end_gfx_init();
                        // 判断存储的输入数字转换为整数后是否在 0 到 180 度的有效范围内
                        if (num.toInt() >= 0 && num.toInt() <= 180) {
                            // 在串口打印输入的数字
                            Serial.println(num);
                            // 初始化字符串 num1，用于显示角度信息
                            num1 = "Angle:";
                            // 控制舵机旋转到输入数字对应的角度
                            ...
                            // 读取舵机当前的角度
                            value = myservo.read();
                            // 将读取到的角度值添加到 num1 字符串中
                            num1 += String(value);
                            // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                            gfx.setCursor(0, 0);
                            // 打印一串空格，用于清空之前在该位置显示的内容
                            gfx.print("             ");
                            // 重新设置文本输出的起始光标位置为屏幕左上角 (0, 0)
                            gfx.setCursor(0, 0);
                            // 在屏幕上显示当前舵机的角度信息
                            gfx.print(num1);
                        } else {
                    ...
                    case 0x16:
                        ...
                        // 将数字字符 '0' 添加到存储输入数字的字符串 num 中
                        num = num + '0';
                        // 调用 print_num 函数，在屏幕上显示当前输入的数字
                        print_num();
                        break;
                    case 0x19:
                ...
                // 如果接收到的是未知协议，或者 DEBUG_BUTTON_PIN 引脚为低电平
                if (IrReceiver.decodedIRData.protocol == UNKNOWN || digitalRead(DEBUG_BUTTON_PIN) == LOW) {
                    // 可选择打印更多原始信息，这里注释掉了打印语句
                    // IrReceiver.printIRResultRawFormatted(&Serial, true);
                }
            }

            // 允许接收下一个红外数据值，准备处理新的红外信号
            IrReceiver.resume();

            // 根据接收到的地址和命令执行其他动作
            if (IrReceiver.decodedIRData.address == 0) {
                if (IrReceiver.decodedIRData.command == 0x10) {
                    // 当地址为 0 且命令为 0x10 时，可执行特定操作，这里留空待扩展
                    // do something
                } else if (IrReceiver.decodedIRData.command == 0x11) {
                    // 当地址为 0 且命令为 0x11 时，可执行特定操作，这里留空待扩展
                    // do something else
                }
            }
        }

        // 短暂延迟 10 毫秒，防止程序过度占用 CPU 资源，让 CPU 有时间处理其他任务
        delay(10);
    }
}
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L14 课程模式
    if (Curriculum_flag == 1) {
        // 调用 L14_Curriculum 函数，执行 L14 课程的具体逻辑，如通过红外遥控器控制舵机转动等
        L14_Curriculum();
    }
