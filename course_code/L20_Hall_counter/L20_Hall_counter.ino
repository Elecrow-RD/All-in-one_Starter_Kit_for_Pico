...
// L20 课程的初始化函数，用于初始化硬件引脚和设置显示信息
void L20_init()
{
  // 将霍尔传感器连接的引脚设置为输入模式，用于读取霍尔传感器的输出信号
  pinMode(hall, INPUT);
  // 将红色 LED 连接的引脚设置为输出模式，用于控制红色 LED 的亮灭
  pinMode(redLed, OUTPUT);
  // 设置图形显示的文本光标位置，坐标为 (100, 100)，字体大小相关参数为 4
  gfx.setCursor(100, 100, 4);
  // 设置图形显示的文本大小为 1
  gfx.setTextSize(1);
  // 在屏幕上打印初始的计数信息，初始计数为 0
  gfx.printf("counting: 0");
  // 将引脚 14 设置为输入模式，用于检测退出课程的信号
  pinMode(14, INPUT);
}

// L20 课程的主逻辑函数，实现使用霍尔传感器计数并控制 LED 闪烁的功能
void L20_Curriculum()
{
  // 使用黑色填充整个屏幕，清空之前的显示内容
  gfx.fillScreen(TFT_BLACK);
  // 调用 L20 课程的初始化函数，完成硬件引脚和显示信息的初始化
  L20_init();

  // 进入一个无限循环，持续检测霍尔传感器的信号和退出信号
  while (1)
  {
    // 延时 10 毫秒，控制循环的执行频率，避免过度占用 CPU 资源
    delay(10);
    ...
    {
      // 进行消抖处理，延时 50 毫秒，避免因信号抖动产生误判
      delay(50);
      // 再次检测霍尔传感器的输出信号是否仍为低电平
      if (digitalRead(hall) == LOW)
      {
        // 等待霍尔传感器的输出信号变为高电平，即等待霍尔传感器释放
        while (digitalRead(hall) == LOW)
          ;
        // 计数变量 count 加 1，表示检测到一次有效的霍尔信号
        count++;
        // 定义一个字符数组，用于存储格式化后的计数信息
        char buffer[50];
        // 将计数信息格式化为字符串，例如 "counting: 1"
        sprintf(buffer, "counting: %d", count);
        // 使用黑色填充整个屏幕，清空之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 设置图形显示的文本光标位置，坐标为 (100, 100)，字体大小相关参数为 4
        gfx.setCursor(100, 100, 4);
        ...
        // 在屏幕上打印更新后的计数信息
        gfx.printf(buffer);
        // 将红色 LED 连接的引脚置为高电平，点亮红色 LED
        digitalWrite(redLed, HIGH);
        // 延时 200 毫秒，使红色 LED 保持点亮状态
        delay(200);
        // 将红色 LED 连接的引脚置为低电平，熄灭红色 LED
        digitalWrite(redLed, LOW);
      }
    }

    // 此处重复了之前检测霍尔传感器信号的代码，可能是代码冗余
    // 检测霍尔传感器的输出信号是否为低电平
    if (digitalRead(hall) == LOW)
    {
      // 进行消抖处理，延时 50 毫秒，避免因信号抖动产生误判
...
// setup 函数在程序启动时仅执行一次，用于完成硬件和软件环境的初始化操作
void setup(void)
{
    // 初始化串口通信，设置波特率为 115200，可用于调试信息输出或与其他设备通信
    Serial.begin(115200);

    // 将 LED_EN 引脚设置为输出模式，该引脚可能用于控制外部 LED 灯的使能
    pinMode(LED_EN, OUTPUT);

    // 初始化 NeoPixel 灯带对象，为后续控制灯带做准备
    strip.begin();
    // 立即关闭 NeoPixel 灯带的所有像素，确保灯带初始状态为熄灭
    strip.show();
    // 设置 NeoPixel 灯带的亮度，亮度值由 BRIGHTNESS 变量指定
    ...
    Wire.setSCL(5);
    // 启动 I2C 通信，使设备可以通过 I2C 总线与其他设备进行数据交互
    Wire.begin();
    // 延时 100 毫秒，给 I2C 设备足够的时间完成初始化
    delay(100);

    // 初始化图形显示设备，为后续的图形和文本显示做准备
    gfx.init();
    ...
    // 设置显示的水平分辨率，具体数值由 screenWidth 变量决定
    disp_drv.hor_res = screenWidth;
    // 设置显示的垂直分辨率，具体数值由 screenHeight 变量决定
...
// loop 函数是程序的主循环，会不断重复执行，用于处理各种周期性任务和事件
void loop()
{
    // 调用 LittlevGL 的定时器处理函数，用于处理界面的定时任务，如动画更新、界面刷新等
    lv_timer_handler();

    // 调用 Scan_button 函数，用于扫描按键状态，检测是否有按键被按下或释放，以响应用户操作
    Scan_button();

    // 延时 10 毫秒，控制循环的执行频率，避免 CPU 资源被过度占用，同时也可适当降低功耗
    delay(10);

    // 判断课程标志位 Curriculum_flag 的值，如果为 1，表示当前处于 L20 课程模式
    if (Curriculum_flag == 1)
    {
        // 调用 L20_Curriculum 函数，执行 L20 课程的具体逻辑，如使用霍尔传感器计数等操作
        L20_Curriculum();
    }
    else
    {
        // 如果不处于课程模式，将红色 LED 对应的引脚置为低电平，即关闭红色 LED
        digitalWrite(redLed, LOW);
    }

    // 判断游戏 1 标志位 game_1 的值，如果为 1，表示当前要进入游戏 1 模式
    if (game_1 == 1)
    {
        // 使用黑色填充整个屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);

        // 将引脚 0 置为低电平，可能用于关闭某个与该引脚关联的外部设备或功能模块
        digitalWrite(0, LOW);

        // 调用 game_1_test 函数，执行游戏 1 的具体逻辑，如游戏的画面渲染、逻辑判断等
        game_1_test();

        // 将引脚 0 置为高电平，可能用于开启某个与该引脚关联的外部设备或功能模块，也可能用于强制刷新显示
        digitalWrite(0, HIGH);

        // 调用 LittlevGL 的定时器处理函数，确保界面更新操作完成
        lv_timer_handler();

        // 使当前屏幕无效，强制刷新界面显示，避免出现黑屏或显示异常的问题
        lv_obj_invalidate(lv_scr_act());

        // 清除 ui_Switch6 控件的选中状态，将其恢复到未选中状态，可能用于重置游戏 1 的相关设置
        lv_obj_clear_state(ui_Switch6, LV_STATE_CHECKED);
    }

    // 判断游戏 2 标志位 game_2 的值，如果为 1，表示当前要进入游戏 2 模式
    if (game_2 == 1)
    {
        // 使用黑色填充整个屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        ...
        // 调用 game_2_test 函数，执行游戏 2 的具体逻辑，如游戏的玩法逻辑、画面绘制等
        game_2_test();

        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);

        // 调用 ui_init 函数，重新初始化用户界面，可能是为了将界面恢复到初始状态或加载游戏 2 对应的界面元素
        ui_init();

        // 切换到 ui_Screen2 界面，使用淡入动画效果，动画时长为 500 毫秒，同时执行界面初始化回调函数
        _ui_screen_change(&ui_Screen2, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_Screen2_screen_init);

        // 调用 LittlevGL 的定时器处理函数，处理界面的定时任务，确保界面更新正常
        lv_timer_handler();

        // 设置 ui_TabView2 控件的活动标签页为第 2 页，并开启动画效果，实现标签页的切换
        lv_tabview_set_act(ui_TabView2, 2, LV_ANIM_ON);

        // 再次调用 LittlevGL 的定时器处理函数，确保界面更新操作完成
        lv_timer_handler();

        // 使当前屏幕无效，强制刷新界面显示，避免出现黑屏或显示异常的问题
        lv_obj_invalidate(lv_scr_act());

        // 注释掉的代码，原本可能用于清除 ui_Switch7 控件的选中状态
    ...
    // 判断游戏 3 标志位 game_3 的值，如果为 1，表示当前要进入游戏 3 模式
    if (game_3 == 1)
    {
        // 使用黑色填充整个屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        ...
        // 调用 game_3_test 函数，执行游戏 3 的具体逻辑，如游戏的规则逻辑、角色控制等
        game_3_test();

        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);

        // 调用 ui_init 函数，重新初始化用户界面，可能是为了将界面恢复到初始状态或加载游戏 3 对应的界面元素
        ui_init();
