...
// BH1750光传感器对象，设置I2C地址为0x5c
BH1750 lightMeter(0x5c);
// 使用预处理指令 #define 定义一个宏 I2C_SDA，将其值设定为 2。
// I2C_SDA 代表 I2C 通信协议中的串行数据线（Serial Data Line），
// 这里指定了该数据线连接到微控制器的 2 号引脚，后续代码中使用 I2C_SDA 时，
// 编译器会自动将其替换为 2，方便代码的维护和修改。
#define I2C_SDA 2

// 同样使用 #define 定义宏 I2C_SCL，值为 3。
// I2C_SCL 是 I2C 通信中的串行时钟线（Serial Clock Line），
// 意味着该时钟线连接到微控制器的 3 号引脚，在 I2C 通信过程中，
// 时钟线用于同步数据传输的时序。
#define I2C_SCL 3 

// 定义宏 Red_LED，值为 18。
// Red_LED 代表红色 LED 灯，表明红色 LED 连接到微控制器的 18 号引脚，
// 在后续的代码里可以利用这个宏来控制红色 LED 的亮灭等状态。
#define Red_LED 18
...
// L6 课程的初始化函数，用于对相关硬件进行初始化设置
void L6_init() {
    // 初始化 I2C 总线，由于 BH1750 库不会自动执行此操作，需要手动设置
    // 设置 I2C 总线的 SDA（数据）引脚为之前定义的 I2C_SDA
    Wire1.setSDA(I2C_SDA);
    // 设置 I2C 总线的 SCL（时钟）引脚为之前定义的 I2C_SCL
    Wire1.setSCL(I2C_SCL);
    // 启动 I2C 总线通信
    Wire1.begin();

    // 初始化光传感器，使用连续高分辨率模式
    // lightMeter.begin 尝试以指定模式和地址初始化光传感器，并指定使用的 I2C 总线为 Wire1
    if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE, 0x5c, &Wire1)) {
        // 若初始化成功，通过串口输出初始化成功信息
    ...
    // 将红色 LED 对应的引脚设置为输出模式，以便后续控制其亮灭
    pinMode(Red_LED, OUTPUT);
    // 将引脚 14 设置为输入模式，可能用于接收外部的控制信号
    pinMode(14, INPUT);
}

// L6 课程的主逻辑函数，实现根据光照强度控制 LED 灯的功能
void L6_Curriculum() {
    // 调用 L6 课程的初始化函数，完成硬件的初始化
    L6_init();
    // 进入一个无限循环，持续执行光照强度检测和 LED 控制逻辑
    while (1) {
        // 检查光传感器的测量结果是否准备好
        if (lightMeter.measurementReady(true)) {
            // 若测量结果准备好，读取当前的光照强度，单位为勒克斯（lux）
            ...
            // 根据光照强度控制 LED 状态
            // 光照强度阈值设为 100lx，可根据实际需求调整
            if (lux < 100) {
                // 若光照强度小于 100lx，点亮红色 LED
                digitalWrite(Red_LED, HIGH);
            } else {
                // 若光照强度大于等于 100lx，熄灭红色 LED
                digitalWrite(Red_LED, LOW);
            }
        }
        // 延时 10 毫秒，避免过于频繁地进行检测和控制
        delay(10);
        // 检测引脚 14 的输入信号
        if (digitalRead(14)) {
            ...
            // 跳出无限循环，结束 L6 课程的逻辑执行
            break;
        }
    }
}

// 程序的初始化函数，在程序启动时执行一次，完成各种硬件和软件的初始化设置
void setup(void) {
    // 初始化串口通信，设置波特率为 115200，用于调试信息的输出
    Serial.begin(115200);
    // 将 LED_EN 引脚设置为输出模式，可能用于控制某个 LED 灯的使能
    ...
    // 设置 I2C 总线的 SDA 引脚为 4
    Wire.setSDA(4);
    // 设置 I2C 总线的 SCL 引脚为 5
    Wire.setSCL(5);
    // 启动 I2C 总线通信
    Wire.begin();
    // 延时 100 毫秒，等待 I2C 总线初始化完成
    delay(100);
...
// 程序的主循环函数，在 setup 函数执行完成后会不断循环执行，处理各种周期性任务
void loop() {
    // 调用 LittlevGL 的定时器处理函数，处理界面的定时更新和动画等操作
    lv_timer_handler();
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L6 课程逻辑
    if (Curriculum_flag == 1) {
        // 将屏幕填充为黑色，清空屏幕显示内容
        gfx.fillScreen(TFT_BLACK);
        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);
        // 调用 L6_Curriculum 函数，执行 L6 课程的具体逻辑
        L6_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    } else {
