...
// 定义超声波传感器引脚部分
// 定义一个常量变量 triggerPin，类型为 byte（无符号 8 位整数），其值为 9。
// 该变量用于表示超声波传感器的触发引脚。触发引脚用于向超声波传感器发送触发信号，
// 当此引脚输出一个短脉冲时，超声波传感器会发射超声波信号。
const byte triggerPin = 9;

// 定义一个常量变量 echoPin，类型为 byte（无符号 8 位整数），其值为 8。
// 该变量用于表示超声波传感器的回声引脚。回声引脚用于接收超声波遇到障碍物后反射回来的回声信号，
// 通过测量回声信号的持续时间，可以计算出障碍物与传感器之间的距离。
const byte echoPin = 8;

// 创建一个 UltraSonicDistanceSensor 类的对象 distanceSensor。
// UltraSonicDistanceSensor 类应该是一个自定义类，用于操作超声波传感器。
// 在创建对象时，将前面定义的触发引脚 triggerPin 和回声引脚 echoPin 作为参数传递给构造函数，
// 这样对象 distanceSensor 就知道要使用哪两个引脚与超声波传感器进行通信，
// 后续可以通过调用该对象的方法来测量距离。
UltraSonicDistanceSensor distanceSensor(triggerPin, echoPin);

// 定义振动马达引脚部分
// 定义一个常量变量 vibratePin，类型为 int（通常为有符号 16 位整数），其值为 15。
// 该变量用于表示振动马达连接到微控制器的引脚。后续可以通过对这个引脚输出不同的电平，
// 来控制振动马达的开启和关闭。
...
// L8 课程的主逻辑函数，实现根据超声波传感器测量的距离控制振动马达的功能
void L8_Curriculum() {
    // 将振动马达连接的引脚设置为输出模式，以便能够控制振动马达的状态
    pinMode(vibratePin, OUTPUT);
    // 初始时将振动马达引脚置为低电平，确保振动马达处于关闭状态
    digitalWrite(vibratePin, LOW);
    // 将引脚 14 设置为输入模式，可能用于检测外部的退出信号
    pinMode(14, INPUT);
    // 使用黑色填充屏幕，清空之前的显示内容
    gfx.fillScreen(TFT_BLACK);

    // 进入一个无限循环，持续进行距离测量和振动马达控制
    while (1) {
        // 调用超声波传感器对象的 measureDistanceCm 方法，测量当前的距离，单位为厘米
        float distance = distanceSensor.measureDistanceCm();
        // 检查测量得到的距离是否大于 0，如果大于 0 表示测量有效
        if (distance > 0) {
            // 判断测量得到的距离是否小于 30 厘米
            if (distance < 30) {
                // 如果距离小于 30 厘米，将振动马达引脚置为高电平，开启振动马达
                digitalWrite(vibratePin, HIGH);
            } else {
                // 如果距离大于等于 30 厘米，将振动马达引脚置为低电平，关闭振动马达
                digitalWrite(vibratePin, LOW);
            }
            // 通过串口打印测量得到的距离，方便调试和监控
        ...
        if (digitalRead(14)) {
            ...
            Curriculum_flag = 0;
            // 跳出当前的无限循环，结束 L8 课程的逻辑执行
            break;
        }
    }
}

// 程序的初始化函数，在程序启动时执行一次，用于初始化各种硬件和软件资源
void setup(void) {
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L8 课程逻辑
    if (Curriculum_flag == 1) {
        // 将引脚 0 置为低电平，关闭背光
        digitalWrite(0, LOW);
        // 调用 L8_Curriculum 函数，执行 L8 课程的具体逻辑
        L8_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    }
