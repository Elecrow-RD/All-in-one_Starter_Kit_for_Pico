...
// L5 课程初始化函数，对相关硬件和显示进行初始化设置
void L5_init() {
    // 将红色 LED 对应的引脚设置为输出模式，以便控制红色 LED 的亮灭
    pinMode(Red_LED, OUTPUT);
    // 将黄色 LED 对应的引脚设置为输出模式，以便控制黄色 LED 的亮灭
    pinMode(Yellow_LED, OUTPUT);
    // 将绿色 LED 对应的引脚设置为输出模式，以便控制绿色 LED 的亮灭
    ...
    // 设置后续要显示在屏幕上的文本大小为 2
    gfx.setTextSize(2);
    // 将引脚 14 设置为输入模式，用于检测外部输入信号
    pinMode(14, INPUT);
}

// L5 课程主逻辑函数，模拟交通灯的工作流程并在屏幕上显示倒计时
void L5_Curriculum() {
    // 调用 L5 课程的初始化函数，进行硬件和显示的初始化
    L5_init();
    // 将课程标志位设置为 0，表示课程开始执行
    Curriculum_flag = 0;
    // 进入一个无限循环，不断循环执行交通灯的各个阶段
    while (1) {
        // 绿灯阶段开始
        // 将绿色 LED 对应的引脚置为高电平，点亮绿色 LED
        digitalWrite(Green_LED, HIGH);
        // 使用黑色填充屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
        gfx.setCursor(0, 0);
        // 在屏幕上打印倒计时文本 "Countdown: 30s"
        gfx.print("Countdown: 30s");
        // 记录当前时间（自程序启动以来的毫秒数），作为绿灯倒计时的起始时间
        start_time = millis();
        // 初始化绿灯剩余时间为 30 秒
        remaining_time = 30;
        // 只要绿灯剩余时间大于 0，就持续循环更新倒计时
        while (remaining_time > 0) {
            // 当绿灯剩余时间小于等于 5 秒时
            if (remaining_time <= 5) {
                // 利用 millis() 函数获取当前时间并对 1000 取模，实现每 1 秒内前半秒亮后半秒灭，使绿灯闪烁
                digitalWrite(Green_LED, (millis() % 1000 < 500));
            }
            // 计算绿灯剩余时间，用总时长 30 秒减去已过去的时间（已过去的时间通过当前时间减去起始时间并转换为秒得到）
            remaining_time = 30 - (millis() - start_time) / 1000;

            // 重新设置文本输出的起始光标位置为屏幕左上角 (0, 0)
            gfx.setCursor(0, 0);
            // 用黑色填充一个指定矩形区域（120, 0 为矩形左上角坐标，100 为宽度，20 为高度），用于局部清屏更新倒计时数字
            gfx.fillRect(120, 0, 100, 20, TFT_BLACK);
            // 设置更新后的倒计时数字的输出光标位置
            gfx.setCursor(130, 0);
            // 打印绿灯剩余时间数字
            gfx.print(remaining_time);
            // 打印秒的单位 "s"
            gfx.print("s");
            // 短暂延迟 100 毫秒，避免更新过快
            delay(100);
            ...
            if (digitalRead(14)) {
                ...
                if (digitalRead(14)) {
                    ...
                    // 结束 L5_Curriculum 函数的执行
                    return;
                }
            }
        }
        // 绿灯倒计时结束，将绿色 LED 对应的引脚置为低电平，熄灭绿色 LED
        digitalWrite(Green_LED, LOW);

        // 黄灯阶段开始
        // 将黄色 LED 对应的引脚置为高电平，点亮黄色 LED
        digitalWrite(Yellow_LED, HIGH);
        // 使用黑色填充屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
        gfx.setCursor(0, 0);
        // 在屏幕上打印倒计时文本 "Countdown: 3s"
        gfx.print("Countdown: 3s");
        // 记录当前时间（自程序启动以来的毫秒数），作为黄灯倒计时的起始时间
        start_time = millis();
        // 初始化黄灯剩余时间为 3 秒
        remaining_time = 3;
        // 只要黄灯剩余时间大于 0，就持续循环更新倒计时
        while (remaining_time > 0) {
            // 计算黄灯剩余时间，用总时长 3 秒减去已过去的时间（已过去的时间通过当前时间减去起始时间并转换为秒得到）
            remaining_time = 3 - (millis() - start_time) / 1000;
        ...
        // 黄灯倒计时结束，将黄色 LED 对应的引脚置为低电平，熄灭黄色 LED
        digitalWrite(Yellow_LED, LOW);

        // 红灯阶段开始
        // 将红色 LED 对应的引脚置为高电平，点亮红色 LED
        digitalWrite(Red_LED, HIGH);
        // 使用黑色填充屏幕，清空屏幕上之前的显示内容
        gfx.fillScreen(TFT_BLACK);
        // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
        gfx.setCursor(0, 0);
        // 在屏幕上打印倒计时文本 "Countdown: 20s"
        gfx.print("Countdown: 20s");
        // 记录当前时间（自程序启动以来的毫秒数），作为红灯倒计时的起始时间
        start_time = millis();
        // 初始化红灯剩余时间为 20 秒
        remaining_time = 20;
        // 只要红灯剩余时间大于 0，就持续循环更新倒计时
        while (remaining_time > 0) {
            // 当红灯剩余时间小于等于 5 秒时
            if (remaining_time <= 5) {
                // 利用 millis() 函数获取当前时间并对 1000 取模，实现每 1 秒内前半秒亮后半秒灭，使红灯闪烁
                digitalWrite(Red_LED, (millis() % 1000 < 500));
            }
            // 计算红灯剩余时间，用总时长 20 秒减去已过去的时间（已过去的时间通过当前时间减去起始时间并转换为秒得到）
            remaining_time = 20 - (millis() - start_time) / 1000;
        ...
        // 红灯倒计时结束，将红色 LED 对应的引脚置为低电平，熄灭红色 LED
...
void loop() {
    ...
    // 延时 10 毫秒，控制循环的执行频率
    delay(10);
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则执行 L5 课程逻辑
    if (Curriculum_flag == 1) {
        // 调用 L5_Curriculum 函数，执行 L5 课程的具体逻辑
        L5_Curriculum();
        // 将红色 LED 对应的引脚置为低电平，熄灭红色 LED
        digitalWrite(Red_LED, LOW);
        // 将绿色 LED 对应的引脚置为低电平，熄灭绿色 LED
        digitalWrite(Green_LED, LOW);
        // 将黄色 LED 对应的引脚置为低电平，熄灭黄色 LED
        digitalWrite(Yellow_LED, LOW);
    }
