...
// L15 课程的初始化函数，在课程开始时运行一次，用于初始化显示和硬件引脚状态
void L15_init() {
    ...
    gfx.setCursor(0, 0);
    // 在屏幕上打印 "Please open the door" 文本，提示用户开门
    gfx.print("Please open the door");

    // 将继电器控制引脚设置为输出模式，以便控制继电器的开关
    pinMode(relayPin, OUTPUT);
    // 将触摸按钮引脚设置为输入模式，用于检测触摸按钮的状态
    pinMode(Touch_pin, INPUT);
    // 初始状态下，将继电器控制引脚置为低电平，即关闭继电器
    digitalWrite(relayPin, LOW);
    // 将引脚 27 设置为输入模式，可能用于检测某个模拟信号
    pinMode(27, INPUT);
}

// L15 课程的主逻辑函数，实现通过触摸按钮控制继电器开关，并在屏幕上显示相应提示信息
void L15_Curriculum() {
    // 调用 L15 课程的初始化函数，完成显示和硬件引脚的初始化
    L15_init();
    // 定义一个整型变量 adcValue，用于存储从引脚 27 读取的模拟值，初始值为 0
    int adcValue = 0;

    // 进入一个无限循环，持续检测触摸按钮状态和模拟输入值
    while (1) {
        // 从引脚 27 读取模拟值，并将其存储到 adcValue 变量中
        adcValue = analogRead(27);
        // 判断读取到的模拟值是否在 800 到 810 之间
        if (adcValue >= 800 && adcValue <= 810) {
            ...
            Curriculum_flag = 0;
            // 跳出当前的无限循环，结束 L15 课程的逻辑执行
            break;
        }

        // 检测触摸按钮引脚的输入信号，如果为高电平，表示触摸按钮被按下
        if (digitalRead(Touch_pin)) {
            ...
            // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
            gfx.setCursor(0, 0);
            // 在屏幕上打印 "Welcome" 文本，欢迎用户
            gfx.print("Welcome");
            // 延时 10000 毫秒（即 10 秒），保持欢迎信息显示一段时间
            delay(10000);
            // 再次使用黑色填充屏幕，清空屏幕上的欢迎信息
            gfx.fillScreen(TFT_BLACK);
            // 将继电器控制引脚置为低电平，关闭继电器
            digitalWrite(relayPin, LOW);
            // 设置文本输出的起始光标位置为屏幕左上角 (0, 0)
            gfx.setCursor(0, 0);
            // 在屏幕上打印 "Please open the door" 文本，再次提示用户开门
            gfx.print("Please open the door");
        }
        // 短暂延迟 100 毫秒，防止循环执行过快占用过多 CPU 资源
        delay(100);
    }
}
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L15 课程模式
    if (Curriculum_flag == 1) {
        // 调用 L15_Curriculum 函数，执行 L15 课程的具体逻辑，如通过触摸按钮控制继电器开关等
        L15_Curriculum();
    }
