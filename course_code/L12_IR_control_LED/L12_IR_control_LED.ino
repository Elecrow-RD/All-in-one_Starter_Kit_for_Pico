...
// 条件编译指令，判断微控制器的闪存结束地址（FLASHEND）是否小于等于 0x1FFF
// 这通常用于判断特定类型的微控制器，根据不同的微控制器类型来决定是否排除某些协议
#if FLASHEND <= 0x1FFF
    // 定义宏 EXCLUDE_EXOTIC_PROTOCOLS，用于在后续代码中排除一些特殊的红外协议
    // 这可能是为了节省程序内存，因为这些特殊协议在某些资源受限的微控制器上可能不需要
...
// APPLICATION_PIN 可能是用户自定义的一个应用引脚，用于特定的功能
    ...
    // DEBUG_BUTTON_PIN 用于控制是否打印接收数据的时间信息
...
// 用于存储上一次接收到红外指令的时间，初始值为 0
// 可以通过这个时间来判断两次红外指令接收的间隔
unsigned long lastReceivedTime = 0;
// 用于存储上一次接收到的红外指令，初始值为 0
// 可以根据这个指令来执行相应的操作
uint16_t lastCommand = 0;
// 定义一个标志变量 run1_flag，初始值为 0
// 可能用于控制某个特定的运行状态或流程
int run1_flag = 0;
// 定义一个标志变量 run2_flag，初始值为 0
// 可能用于控制另一个特定的运行状态或流程
int run2_flag = 0;

// 用于存储上一次执行特定操作的时间，初始值为 0
// 结合时间间隔常量，可以实现定时执行某些操作
unsigned long previousMillis = 0;
// 定义一个时间间隔常量 inter1，值为 1000 毫秒（即 1 秒）
// 可能用于控制某些操作每隔 1 秒执行一次
const long inter1 = 1000;
// 定义一个时间间隔常量 inter12，值为 500 毫秒（即 0.5 秒）
// 可能用于控制某些操作每隔 0.5 秒执行一次
const long inter12 = 500;
...
// run1函数，用于执行特定LED闪烁逻辑
void run1() {
    // 获取当前时间，单位为毫秒
    unsigned long currentMillis = millis();
    // 检查当前时间与上一次执行操作的时间差值是否达到设定的延迟时间（inter1，这里为1000毫秒）
    if (currentMillis - previousMillis >= inter1) {
        // 读取红色LED当前的状态
        int redState = digitalRead(Red_LED);
        // 反转红色LED的状态，若原来是高电平则变为低电平，反之亦然
        digitalWrite(Red_LED, !redState);
        // 读取黄色LED当前的状态
        int yellowState = digitalRead(Yellow_LED);
        // 反转黄色LED的状态
        digitalWrite(Yellow_LED, !yellowState);
        // 读取绿色LED当前的状态
        int greenState = digitalRead(Green_LED);
        // 反转绿色LED的状态
        digitalWrite(Green_LED, !greenState);
        // 更新上一次执行操作的时间为当前时间，为下一次时间判断做准备
        previousMillis = currentMillis;
        // 通过串口打印run1_flag的值，可用于调试观察该标志位状态
        Serial.println(run1_flag);
    }
}

// run2函数，用于执行特定LED切换逻辑
void run2() {
    // 获取当前时间，单位为毫秒
    unsigned long currentMillis = millis();
    // 检查当前时间与上一次执行操作的时间差值是否达到设定的延迟时间（inter12，这里为500毫秒）
    if (currentMillis - previousMillis >= inter12) {
        // 读取红色LED当前的状态
        int redState = digitalRead(Red_LED);
        // 读取黄色LED当前的状态
        int yellowState = digitalRead(Yellow_LED);
...
// L12课程的主逻辑函数
void L12_Curriculum() {
    // 如果Flash内存为16k及以上（通过判断FLASHEND是否大于等于0x3FFF），如ATtiny1604
    // 将DEBUG_BUTTON_PIN引脚配置为输入上拉模式
    #if FLASHEND >= 0x3FFF
        pinMode(DEBUG_BUTTON_PIN, INPUT_PULLUP);
    #endif
    // 初始化红外接收器，使用引脚11接收红外信号，并启用LED反馈功能
    IrReceiver.begin(11, ENABLE_LED_FEEDBACK);
    // 打印红外发送用法信息到串口，方便调试
    IrReceiver.printIRSendUsage(&Serial);

    // 将红色LED引脚设置为输出模式，以便控制其亮灭
    ...
    while (1) {
            ...
            if (IrReceiver.decodedIRData.flags & IRDATA_FLAGS_WAS_OVERFLOW) {
                ...
                switch (IrReceiver.decodedIRData.command) {
                    ...
                    case 0x0D:
                        ...
                        break;
                    case 0x0C:
                        // 当接收到指令 0x0C 时，点亮红色 LED，关闭黄色和绿色 LED
                        digitalWrite(Red_LED, HIGH);
                        ...
                        break;
                    case 0x18:
                        // 当接收到指令 0x18 时，点亮黄色 LED，关闭红色和绿色 LED
                        ...
                        break;
                    case 0x5E:
                        // 当接收到指令 0x5E 时，点亮绿色 LED，关闭红色和黄色 LED
                        digitalWrite(Red_LED, LOW);
                        digitalWrite(Yellow_LED, LOW);
                        digitalWrite(Green_LED, HIGH);
                        ...
                        break;
                    case 0x08:
                        // 当接收到指令 0x08 时，关闭所有 LED，并将 run1_flag 置为 1
                        digitalWrite(Red_LED, LOW);
                        digitalWrite(Yellow_LED, LOW);
                        digitalWrite(Green_LED, LOW);
                        run1_flag = 1;
                        ...
                        break;
                    case 0x1C:
                        // 当接收到指令 0x1C 时，点亮红色 LED，关闭黄色和绿色 LED，并将 run2_flag 置为 1
                        run2_flag = 1;
                        digitalWrite(Red_LED, HIGH);
                        digitalWrite(Yellow_LED, LOW);
                        digitalWrite(Green_LED, LOW);
        ...
        // 如果 run1_flag 标志位为 1，调用 run1 函数执行 LED 闪烁逻辑
        if (run1_flag)
            run1();
        // 如果 run2_flag 标志位为 1，调用 run2 函数执行 LED 切换逻辑
        if (run2_flag)
            run2();
...
// 程序初始化函数，在程序开始运行时仅执行一次，用于对硬件和软件环境进行初始化设置
void setup(void) {
    // 初始化串口通信，设置波特率为 115200，用于与外部设备进行数据传输和调试信息输出
    Serial.begin(115200);

    // 将 LED_EN 引脚设置为输出模式，此引脚可能用于控制某个 LED 灯的使能
    ...
    for (int i = 0; i <= 100; i++) {
        ...
        lv_timer_handler();
        // 延时 50 毫秒，控制进度条的加载速度，使进度条缓慢增长
        delay(50);
    }
    // 切换到 ui_Screen2 界面，使用淡入动画效果，动画时长为 500 毫秒
    _ui_screen_change(&ui_Screen2, LV_SCR_LOAD_ANIM_FADE_ON, 500, 0, &ui_Screen2_screen_init);
    // 再次调用 LittlevGL 的定时器处理函数，确保界面更新完成，避免界面显示异常
    lv_timer_handler();
}
...
void loop() {
    ...
    // 判断课程标志位 Curriculum_flag 是否为 1，如果为 1 则表示进入 L12 课程模式
    if (Curriculum_flag == 1) {
        ...
        pinMode(14, INPUT);
        // 调用 L12_Curriculum 函数，执行 L12 课程的具体逻辑，如红外遥控控制 LED 等操作
        L12_Curriculum();
        // 将引脚 0 置为高电平，开启背光
        digitalWrite(0, HIGH);
    ...
    // 如果课程标志位 Curriculum_flag 不为 1，则关闭红、绿、黄三个 LED 灯
    else {
        digitalWrite(Red_LED, LOW);
        digitalWrite(Green_LED, LOW);
        digitalWrite(Yellow_LED, LOW);
